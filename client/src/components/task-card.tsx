...
import { MapPin, Box, AlertCircle, CheckCircle2, DollarSign, BarChart3 } from "lucide-react";
...
export function TaskCard({ task }: TaskCardProps) {
  ...
  // Determine priority color based on missed sales (simulated logic)
  const missedSales = parseFloat(task.missedSales);
  const isHighPriority = missedSales > 100;

  return (
    <Link href={`/task/${task.uniqueId}`}>

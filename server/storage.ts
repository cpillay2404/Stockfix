...
export interface IStorage {
  ...
  getTrendAnalytics(filters?: { client?: string; region?: string; weeks?: number }): Promise<{
    weeklyTrend: { weekEnding: string; total: number; completed: number; pending: number; completionRate: number }[];
    reasonCodeBreakdown: { reasonCode: string; count: number }[];
    actionTypeBreakdown: { action: string; total: number; completed: number }[];
    regionBreakdown: { region: string; total: number; completed: number; completionRate: number }[];
    stockClassificationTrend: { weekEnding: string; classification: string; count: number }[];
    kpis: { totalTasks: number; totalCompleted: number; avgCompletionRate: number; totalVariance: number; weeksTracked: number };
  }>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getTrendAnalytics(filters?: { client?: string; region?: string; weeks?: number }): Promise<{
    weeklyTrend: { weekEnding: string; total: number; completed: number; pending: number; completionRate: number }[];
    reasonCodeBreakdown: { reasonCode: string; count: number }[];
    actionTypeBreakdown: { action: string; total: number; completed: number }[];
    regionBreakdown: { region: string; total: number; completed: number; completionRate: number }[];
    ...
    if (filters?.client) conditions.push(eq(tasks.client, filters.client));
    if (filters?.region) conditions.push(eq(tasks.region, filters.region));
    const whereClause = conditions.length > 0 ? and(...conditions) : undefined;
    const weeksLimit = filters?.weeks || 12;

    const weeklyRaw = await db
      .select({
        weekEnding: tasks.weekEndingDate,
        total: count(),
        completed: sql<number>`COUNT(*) FILTER (WHERE ${tasks.actionStatus} = 'Completed')`,
      })
      .from(tasks)
      .where(whereClause)
      .groupBy(tasks.weekEndingDate)
    ...
    const reasonCodeRaw = await db
      .select({
        reasonCode: tasks.reasonCode,
        cnt: count(),
      })
      .from(tasks)
      .where(whereClause ? and(whereClause, sql`${tasks.reasonCode} IS NOT NULL AND ${tasks.reasonCode} != ''`) : sql`${tasks.reasonCode} IS NOT NULL AND ${tasks.reasonCode} != ''`)
      .groupBy(tasks.reasonCode)
      .orderBy(desc(count()))
      .limit(10);

    const reasonCodeBreakdown = reasonCodeRaw.map(r => ({
    ...
    const actionTypeBreakdown = actionTypeRaw.map(r => ({
      action: r.action || 'Unknown',
    ...
    const classificationRaw = await db
      .select({
        weekEnding: tasks.weekEndingDate,
        classification: tasks.stockClassification,
        cnt: count(),
      })
      .from(tasks)
      .where(whereClause)
      .groupBy(tasks.weekEndingDate, tasks.stockClassification)
      .orderBy(desc(tasks.weekEndingDate))
      .limit(weeksLimit * 8);

    const recentWeeks = new Set(weeklyTrend.map(w => w.weekEnding));
    const stockClassificationTrend = classificationRaw
      .filter(r => r.weekEnding && recentWeeks.has(r.weekEnding))
      .map(r => ({
        weekEnding: r.weekEnding || '',
    ...
    const [kpiRow] = await db
      .select({
        totalTasks: count(),
        totalCompleted: sql<number>`COUNT(*) FILTER (WHERE ${tasks.actionStatus} = 'Completed')`,
        totalVariance: sql<number>`COALESCE(SUM(ABS(CAST(NULLIF(${tasks.variance}, '') AS NUMERIC))), 0)`,
      })
      .from(tasks)
      .where(whereClause);

    const totalTasks = Number(kpiRow?.totalTasks) || 0;
    const totalCompleted = Number(kpiRow?.totalCompleted) || 0;
    ...
    return {
      ...
      actionTypeBreakdown,
      regionBreakdown,
      stockClassificationTrend,
      kpis: {
        totalTasks,
        totalCompleted,
        avgCompletionRate: totalTasks > 0 ? Math.round((totalCompleted / totalTasks) * 1000) / 10 : 0,
        totalVariance: Math.round(Number(kpiRow?.totalVariance) || 0),
        weeksTracked: weeklyTrend.length,
      },
    };
  }
